...
exports.removeUserFromBoard = exports.inviteUserToBoard = void 0;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
admin.initializeApp();
const db = admin.firestore();
exports.inviteUserToBoard = functions.https.onCall(async (data, context) => {
    ...
    if (!email || !boardId) {
        throw new functions.https.HttpsError("invalid-argument", "Please provide an email and board ID.");
    }
    try {
        ...
        if (!boardData || boardData.owner !== uid) {
            throw new functions.https.HttpsError("permission-denied", "You must be the board owner to invite users.");
        }
        ...
        if (boardData.members && boardData.members.includes(invitedUid)) {
            throw new functions.https.HttpsError("already-exists", "This user is already a member of the board.");
        }
...
exports.removeUserFromBoard = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
